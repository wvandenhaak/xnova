...
require_once dirname(__FILE__) .'/common.php';
    ...
    if ($mode == 0) {
        ...
        if (!isset($_POST["galaxy"]) || $_POST["galaxy"] <= 0) {
            $galaxy = 1;
        } else if ($_POST["galaxy"] >= Constants::MAX_GALAXY_IN_WORLD) {
            $galaxy = Constants::MAX_GALAXY_IN_WORLD;
        } else {
            $galaxy = intval($_POST["galaxy"]) + 1;
        }
        ...
        if (!isset($_POST["system"]) || $_POST["system"] <= 0) {
            $system = 1;
        } else if ($_POST["system"] >= Constants::MAX_SYSTEM_IN_GALAXY) {
            $system = Constants::MAX_SYSTEM_IN_GALAXY;
        } else {
            $system = intval($_POST["system"]) + 1;
        }
        ...
        if (!isset($_POST["planet"]) || $_POST["planet"] <= 0) {
            $planet = 1;
        } else if ($_POST["planet"] >= Constants::MAX_PLANET_IN_SYSTEM) {
            $planet = Constants::MAX_PLANET_IN_SYSTEM;
        } else {
